...
public class ServerListener extends Listener {
    ...
    public ServerListener(ServerMain main) {
        this.main = main;
    }
    ...
    public void received(Connection connection, Object object) {
        if (object instanceof KeyInputC2S(UUID uuid, KeyInputC2S.Action action, int keycode)) {
            CharacterEntity character = main.getGame().getEntityManager().getCharacter(uuid);
            ((CharacterController) character.getController()).onKey(action, keycode);

            main.getServer().sendToTCP(connection.getID(), new CharacterSpawnS2C(uuid, character.getType().textureId(), character.getPos().x, character.getPos().y, character.getType().scale()));
        }
            ...
            Commands.tryExecute(main.getGame(), uuid, commandType, args);
        }
            ...
            CharacterEntity character = main.getGame().getEntityManager().addEntity(uuid, new CharacterEntityType(new Mutable<>(main.getGame().getDataManager().getCharacterClass("wizard"))), new Vector2(0, 0), data);
            main.getServer().sendToTCP(connection.getID(), new CharacterSpawnS2C(uuid, character.getType().textureId(), character.getPos().x, character.getPos().y, character.getType().scale()));
        }

        else if (object instanceof CharacterLeaveC2S(UUID uuid)) {
            main.getGame().getEntityManager().removeEntity(uuid);
        }
    }
}
