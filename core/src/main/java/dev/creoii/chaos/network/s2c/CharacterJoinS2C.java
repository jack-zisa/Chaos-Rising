...
public record CharacterJoinS2C(int id, float x, float y, float scale, EntityCustomData data) {
    public static final Codec<CharacterJoinS2C> CODEC = RecordCodecBuilder.create(instance -> {
        ...
        ).apply(instance, (uuid, x, y, scale, customData) -> new CharacterJoinS2C(uuid, x.orElse(0f), y.orElse(0f), scale, customData));
    });
    ...
    public static CharacterJoinS2C read(Input input) {
        return new CharacterJoinS2C(input.readInt(), input.readFloat(), input.readFloat(), input.readFloat(), PacketUtils.readCustomEntityData(input));
    }
}
