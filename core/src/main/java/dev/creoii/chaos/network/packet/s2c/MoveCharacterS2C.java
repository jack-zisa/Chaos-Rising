...
public record MoveCharacterS2C(float x, float y, float xv, float yv) implements Serializable {
    public static final Codec<MoveCharacterS2C> CODEC = RecordCodecBuilder.create(instance -> {
        ...
        ).apply(instance, (x, y, xv, yv) -> new MoveCharacterS2C(x.orElse(0f), y.orElse(0f), xv.orElse(0f), yv.orElse(0f)));
    });
}
